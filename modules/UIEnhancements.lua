    ...
    Init = function(Context, Scope, _Tab)
        ...
        local function findControl(flag)
            ...
            for _, control in pairs(Library.Controls or {}) do
                if control and control.Flag == flag then return control end
            end
            return nil
        end
        ...
        local function toggleControl(flag, label)
            ...
            pcall(function() control:Set(not current) end)
        ...
        local function findSearchBox()
            ...
            for _, object in ipairs(root:GetDescendants()) do
                if object:IsA("TextBox") then
                    ...
                    if placeholder:find("search", 1, true) or placeholder:find("поиск", 1, true) then return object end
                end
            end
            return nil
        end
        ...
        local function findFavoritesButton()
            ...
            if not root then return nil end
            for _, object in ipairs(root:GetDescendants()) do
                if object:IsA("TextButton") and tostring(object.Text):find("★", 1, true) then
                    local size = object.Size
                    if size.X.Offset <= 48 and size.Y.Offset <= 40 then return object end
                end
            end
            return nil
        end
        ...
        local function focusSearch()
            ...
            if box then pcall(function() box:CaptureFocus() end) else notify("Search field not found", "Warning") end
        end
        ...
        local function toggleMusic()
            ...
            if music and type(music.Toggle) == "function" then music.Toggle() else notify("Music module is not loaded", "Warning") end
        end

        local function validKey(name) return type(name) == "string" and Enum.KeyCode[name] ~= nil end
        local function pressed(input, name)
            return input.UserInputType == Enum.UserInputType.Keyboard and validKey(name) and input.KeyCode == Enum.KeyCode[name]
        end
        ...
        local function refreshFavoriteStar(control, star)
            ...
            local favorite = Library:IsFavorite(control)
        ...
        local function decorateControl(control)
            ...
            star.Position = UDim2.new(0, 5, 0.5, 0)
            star.Size = UDim2.fromOffset(18, 22)
            ...
            if displayName ~= "" then
                for _, child in ipairs(holder:GetDescendants()) do
                    if child:IsA("TextLabel") and child.Text == displayName then
                        ...
                        if p.X.Offset < 28 then child.Position = UDim2.new(p.X.Scale, p.X.Offset + 18, p.Y.Scale, p.Y.Offset) end
                        break
                    end
                end
            end
            ...
            star.MouseButton1Click:Connect(function()
                Library:SetFavorite(control, not Library:IsFavorite(control))
            ...
            while running and not Context.Unloaded do
                ...
                if State.FavoriteStars then
                    for _, control in pairs(Library.Controls or {}) do
                        ...
                        if typeof(star) == "Instance" then refreshFavoriteStar(control, star) end
                    end
                end
                task.wait(0.5)
            end
