...
return {
    ...
    Init = function(Context, Scope, Tab)
        ...
        local State = Context:GetState("Music", {
            Folder = "Experiment17_Visuals/Music",
            Volume = 0.65,
            PlaybackSpeed = 1.0,
            Loop = false,
            Shuffle = false,
        ...
        local function ensureFolder()
            local isfolder=fs("isfolder")
            local makefolder=fs("makefolder")
        ...
        local function refreshStatus(extra)
            ...
            local name=R.Index>0 and R.Tracks[R.Index] and R.Tracks[R.Index].Name or "None"
            local length=Sound.TimeLength or 0
            local text=string.format("%s | %s / %s | %s",name,formatTime(Sound.TimePosition),formatTime(length),Sound.Playing and "PLAYING" or "PAUSED")
        ...
        local function updateChoice()
            ...
            local names={}; for _,track in ipairs(R.Tracks) do names[#names+1]=track.Name end
            if #names==0 then names={"None"} end
            R.Choice:SetValues(names,false)
            if #R.Tracks>0 then
                local selected=R.Selected~="None" and R.Selected or R.Tracks[1].Name
                local found=false; for _,n in ipairs(names) do if n==selected then found=true break end end
                if not found then selected=names[1] end
                R.Selected=selected; pcall(function() R.Choice:Set(selected,true) end)
            else
                R.Selected="None"; pcall(function() R.Choice:Set("None",true) end)
            end
        end
        ...
        local function rescan(silent)
            ...
            if not ok or type(files)~="table" then
                ...
                if not silent then notify("Could not scan music folder: "..State.Folder,"Warning") end
                return false
            end
        ...
        local function nextTrack()
            ...
            local index
            if State.Shuffle and #R.Tracks>1 then repeat index=math.random(1,#R.Tracks) until index~=R.Index else index=(R.Index%#R.Tracks)+1 end
            playIndex(index)
        end
        local function previousTrack()
            if #R.Tracks==0 then return end
            local index=R.Index-1; if index<1 then index=#R.Tracks end
            playIndex(index)
        end
        local function toggle()
            if Sound.Playing then
                pcall(function() Sound:Pause() end); refreshStatus(); notify("Music paused","Info")
            else
                if Sound.SoundId~="" and Sound.TimePosition>0 then
                    local ok=pcall(function() Sound:Resume() end); if not ok then pcall(function() Sound:Play() end) end
                    refreshStatus(); notify("Music resumed","Info")
        ...
        Playback:AddButtonGroup({Name="Playback",RequiredGraphics="Low",Buttons={
            {Text="Play",Callback=playSelected},{Text="Pause / Resume",Callback=toggle},{Text="Prev",Callback=previousTrack},{Text="Next",Callback=nextTrack},
        }})
        ...
        R.Seek=Playback:AddSlider({Name="Seek",Flag="Music_Seek",Min=0,Max=100,Default=0,Decimals=1,Suffix="%",RequiredGraphics="Low",Callback=function(v)
        ...
        Files:AddButtonGroup({Name="Folder Actions",RequiredGraphics="Low",Buttons={
            {Text="Rescan",Callback=function() rescan(false) end},
            {Text="Create Folder",Callback=function() local ok=ensureFolder(); notify(ok and ("Folder ready: "..State.Folder) or "Could not create folder",ok and "Success" or "Warning") end},
        }})
        ...
        Scope:TrackConnection(RunService.Heartbeat:Connect(function(dt)
            timer+=dt; if timer<0.25 then return end; timer=0
            refreshStatus()
            if R.Seek and type(R.Seek.Set)=="function" then
                local len=Sound.TimeLength or 0
                local pct=len>0 and math.clamp((Sound.TimePosition/len)*100,0,100) or 0
                R.UpdatingSeek=true; pcall(function() R.Seek:Set(pct,true) end); R.UpdatingSeek=false
            end
        end))

        task.defer(function() rescan(true) end)
        Scope:AddCleaner(function() R.Running=false; pcall(function() Sound:Stop() end) end)
    end,
}
